...
export async function verifyEmail(token: string): Promise<void> {
  ...
  if (rowCount === 0) throw Object.assign(new Error('Invalid or expired token'), { code: 'INVALID_VERIFY' });
                                                                              ...
                                                                              roblox-ai-platform/backend/src/services/                                                            0000755 0000000 0000000 00000000000 15211423620 017267  5                                                                                                    ustar   root                            root                                                                                                                                                                                                                   roblox-ai-platform/backend/src/services/jobs.service.ts                                             0000644 0000000 0000000 00000010707 15211423620 022240  0                                                                                                    ustar   root                            root                                                                                                                                                                                                                   import { GoogleGenerativeAI } from '@google/generative-ai';
import { db } from '../db/client.js';
import { logger } from '../utils/logger.js';
...
export async function getUserJobHistory(userId: string, limit = 20) {
  ...
  return rows;
                                                         ...
                                                         roblox-ai-platform/backend/src/services/claude.service.ts                                           0000644 0000000 0000000 00000014424 15211423620 022540  0                                                                                                    ustar   root                            root                                                                                                                                                                                                                   // ============================================================
...
-- ─────────────────────────────────────────────
-- TOOLBAR + WIDGET (must be created at top level)
-- ─────────────────────────────────────────────
local toolbar = plugin:CreateToolbar("Lime AI")
local toggleBtn = toolbar:CreateButton(
	"Lime AI",
	"Open Lime AI — AI Coding Assistant for Roblox",
	"rbxassetid://6031068426"
)

local widgetInfo = DockWidgetPluginGuiInfo.new(
	Enum.InitialDockState.Right,
	true, false, 400, 640, 300, 440
)
...
local function addMessage(role, text)
	...
	if hasCode and not isUser then
		...
		while rem:len() > 0 do
			local cs = rem:find("```")
			if cs then
				if cs > 1 then table.insert(parts, {type="text", content=rem:sub(1, cs-1)}) end
				local le = rem:find("\n", cs+3) or cs+3
				local lang = rem:sub(cs+3, le-1)
				local ce = rem:find("```", le)
				if ce then
					table.insert(parts, {type="code", lang=lang, content=rem:sub(le+1, ce-1)})
					rem = rem:sub(ce+3)
				else
					table.insert(parts, {type="text", content=rem:sub(cs)})
					rem = ""
				end
			else
				table.insert(parts, {type="text", content=rem})
				rem = ""
			end
		end
		...
		for i, part in ipairs(parts) do
			if part.type == "text" and part.content:gsub("%s","") ~= "" then
				...
				tf.BackgroundColor3=SURFACE; tf.BorderSizePixel=0
				tf.LayoutOrder=i; tf.Parent=wrapper
				local tc2 = Instance.new("UICorner"); tc2.CornerRadius=UDim.new(0,10); tc2.Parent=tf
				local ts2 = Instance.new("UIStroke"); ts2.Color=BORDER; ts2.Thickness=1; ts2.Parent=tf
				local tp = Instance.new("UIPadding"); tp.PaddingAll=UDim.new(0,10); tp.Parent=tf
				local tl = Instance.new("TextLabel")
				tl.Size=UDim2.new(1,0,0,0); tl.AutomaticSize=Enum.AutomaticSize.Y
				tl.BackgroundTransparency=1; tl.Text=part.content:gsub("^%s+",""):gsub("%s+$","")
				tl.TextColor3=TEXT; tl.Font=Enum.Font.Gotham; tl.TextSize=12
				tl.TextWrapped=true; tl.TextXAlignment=Enum.TextXAlignment.Left; tl.Parent=tf

			elseif part.type == "code" then
				local cf = Instance.new("Frame")
				cf.Size=UDim2.new(1,0,0,0); cf.AutomaticSize=Enum.AutomaticSize.Y
				cf.BackgroundColor3=CODE_BG; cf.BorderSizePixel=0
				cf.LayoutOrder=i; cf.Parent=wrapper
				local cc2 = Instance.new("UICorner"); cc2.CornerRadius=UDim.new(0,10); cc2.Parent=cf
				local cs2 = Instance.new("UIStroke"); cs2.Color=LIME_MID; cs2.Thickness=0.5; cs2.Parent=cf

				-- Header
				local ch = Instance.new("Frame")
				ch.Size=UDim2.new(1,0,0,32); ch.BackgroundColor3=Color3.fromRGB(8,14,6)
				ch.BorderSizePixel=0; ch.Parent=cf
				local chc = Instance.new("UICorner"); chc.CornerRadius=UDim.new(0,10); chc.Parent=ch
				local chb = Instance.new("Frame")
				chb.Size=UDim2.new(1,0,0,10); chb.Position=UDim2.new(0,0,0,22)
				chb.BackgroundColor3=Color3.fromRGB(8,14,6); chb.BorderSizePixel=0; chb.Parent=cf

				local ll = Instance.new("TextLabel")
				ll.Size=UDim2.new(0.5,0,1,0); ll.Position=UDim2.new(0,10,0,0)
				ll.BackgroundTransparency=1; ll.Text=(part.lang~="" and part.lang or "lua"):upper()
				ll.TextColor3=LIME; ll.Font=Enum.Font.Code; ll.TextSize=10
				ll.TextXAlignment=Enum.TextXAlignment.Left; ll.Parent=ch

				-- Insert button
				local ib = Instance.new("TextButton")
				ib.Size=UDim2.new(0,110,0,22); ib.Position=UDim2.new(1,-118,0,5)
				ib.BackgroundColor3=LIME_DARK; ib.Text="⬇ Insert Script"
				ib.TextColor3=LIME; ib.Font=Enum.Font.GothamBold; ib.TextSize=10
				ib.BorderSizePixel=0; ib.Parent=ch
				local ibc = Instance.new("UICorner"); ibc.CornerRadius=UDim.new(0,6); ibc.Parent=ib

				-- Code text
				local cl2 = Instance.new("TextLabel")
				cl2.Size=UDim2.new(1,-20,0,0); cl2.AutomaticSize=Enum.AutomaticSize.Y
				cl2.Position=UDim2.new(0,10,0,38)
				cl2.BackgroundTransparency=1; cl2.Text=part.content
				cl2.TextColor3=CODE_TEXT; cl2.Font=Enum.Font.Code; cl2.TextSize=11
				cl2.TextWrapped=true; cl2.TextXAlignment=Enum.TextXAlignment.Left; cl2.Parent=cf

				local cp3 = Instance.new("UIPadding"); cp3.PaddingBottom=UDim.new(0,10); cp3.Parent=cf

				local code = part.content
				ib.MouseButton1Click:Connect(function()
					insertCode(code, "Script", "ServerScriptService")
					ib.Text = "✓ Inserted!"
					ib.BackgroundColor3 = LIME_MID
					task.delay(2, function()
						ib.Text = "⬇ Insert Script"
						ib.BackgroundColor3 = LIME_DARK
					end)
				end)
			end
		end
		...
		-- Simple text message
		local wl2 = Instance.new("UIListLayout")
		wl2.FillDirection=Enum.FillDirection.Vertical
		wl2.SortOrder=Enum.SortOrder.LayoutOrder
		wl2.Padding=UDim.new(0,4); wl2.Parent=wrapper

		if not isUser then
			local badge2 = Instance.new("Frame")
			badge2.Size=UDim2.new(0,58,0,18); badge2.BackgroundColor3=LIME_DARK
			badge2.BorderSizePixel=0; badge2.LayoutOrder=1; badge2.Parent=wrapper
			local b2c = Instance.new("UICorner"); b2c.CornerRadius=UDim.new(0,4); b2c.Parent=badge2
			local b2l = Instance.new("TextLabel")
			b2l.Size=UDim2.new(1,0,1,0); b2l.BackgroundTransparency=1
			b2l.Text="🟢 Lime AI"; b2l.TextColor3=LIME; b2l.Font=Enum.Font.GothamBold
			b2l.TextSize=9; b2l.TextXAlignment=Enum.TextXAlignment.Center; b2l.Parent=badge2
		end
		...
		bubble.BorderSizePixel=0; bubble.LayoutOrder=2; bubble.Parent=wrapper
		if isUser then
		...
		tl2.Size=UDim2.new(1,0,0,0); tl2.AutomaticSize=Enum.AutomaticSize.Y
		tl2.BackgroundTransparency=1; tl2.Text=text
		tl2.TextColor3=isUser and LIME or TEXT
		tl2.Font=Enum.Font.Gotham; tl2.TextSize=13
		tl2.TextWrapped=true; tl2.TextXAlignment=Enum.TextXAlignment.Left
		tl2.LineHeight=1.45; tl2.Parent=bubble
	end
...
local function sendMessage(message)
	...
	task.spawn(function()
		...
		if err or not result then
			addMessage("assistant", "❌ " .. (err or "Something went wrong. Visit your Render URL in a browser first to wake the server, then try again!"))
		else
...
local function buildMainUI()
	...
	topBar.Name="TopBar"; topBar.Size=UDim2.new(1,0,0,52)
	...
	local lps = Instance.new("UIStroke"); lps.Color=LIME_MID; lps.Thickness=0.5; lps.Parent=logoPill
	...
	local nbs = Instance.new("UIStroke"); nbs.Color=LIME_MID; nbs.Thickness=0.5; nbs.Parent=newBtn

	-- Status bar
	local statusBar = Instance.new("Frame")
	statusBar.Name="StatusBar"; statusBar.Size=UDim2.new(1,0,0,22)
	statusBar.BackgroundColor3=SURFACE2; statusBar.BorderSizePixel=0
	statusBar.LayoutOrder=2; statusBar.Parent=root
	local sbs = Instance.new("UIStroke"); sbs.Color=BORDER; sbs.Thickness=0.5; sbs.Parent=statusBar
	...
	msgOuter.Name="MsgOuter"; msgOuter.Size=UDim2.new(1,0,1,-126)
	...
	inputArea.Name="InputArea"; inputArea.Size=UDim2.new(1,0,0,52)
...
local function buildLoginUI()
	...
	local cs2 = Instance.new("UIStroke"); cs2.Color=BORDER; cs2.Thickness=1; cs2.Parent=card
	...
	local lcs = Instance.new("UIStroke"); lcs.Color=LIME_MID; lcs.Thickness=1; lcs.Parent=logoCircle
	...
	local function makeCardLabel(txt, size, color, font, order)
		local l = Instance.new("TextLabel")
		l.Size=UDim2.new(1,0,0,size); l.BackgroundTransparency=1
		l.Text=txt; l.TextColor3=color; l.Font=font; l.TextSize=size
		l.TextXAlignment=Enum.TextXAlignment.Center; l.LayoutOrder=order; l.Parent=card
		return l
	end

	makeCardLabel("Lime AI", 22, LIME, Enum.Font.GothamBold, 2)
	makeCardLabel("AI coding assistant for Roblox Studio", 11, TEXT_DIM, Enum.Font.Gotham, 3)

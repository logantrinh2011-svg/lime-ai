...
async function processCodeJob(jobId: string, userId: string, prompt: string, scriptType: string): Promise<void> {
  ...
  try {
    ...
    const result = await model.generateContent(`Generate a ${scriptType} for Roblox Studio that does: ${prompt}`);
    const rawText = result.response.text();
    const clean = rawText.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
    const parsed = JSON.parse(clean);
    if (!parsed.code || !parsed.scriptName) throw new Error('Missing code or scriptName');
    await db.query(`UPDATE code_jobs SET status = 'completed', generated_code = $1, explanation = $2, script_name = $3, completed_at = NOW() WHERE id = $4`, [parsed.code, parsed.explanation, parsed.scriptName, jobId]);
    logger.info('Code job completed', { jobId, scriptName: parsed.scriptName });
    await db.query(`INSERT INTO analytics_events (user_id, event_type, properties) VALUES ($1, 'code_job_completed', $2)`, [userId, JSON.stringify({ jobId, scriptType })]);
  } catch (err: unknown) {
    const error = err as Error;
    logger.error('Code job failed', { jobId, error: error.message });
    await db.query(`UPDATE code_jobs SET status = 'failed', error_message = $1 WHERE id = $2`, [error.message, jobId]);
  }
}

export async function getPendingJobsForUser(userId: string) {
  const { rows } = await db.query(`SELECT id, script_name, script_type, insert_location, generated_code, explanation, created_at FROM code_jobs WHERE user_id = $1 AND status = 'completed' ORDER BY created_at ASC LIMIT 10`, [userId]);
  return { jobs: rows.map((r: any) => ({ id: r.id, scriptName: r.script_name, scriptType: r.script_type, insertLocation: r.insert_location, code: r.generated_code, explanation: r.explanation, createdAt: r.created_at })) };
}

export async function markJobInserted(jobId: string, userId: string): Promise<void> {
  await db.query(`UPDATE code_jobs SET status = 'inserted', inserted_at = NOW() WHERE id = $1 AND user_id = $2`, [jobId, userId]);
}

export async function getJobStatus(jobId: string, userId: string) {
  const { rows } = await db.query(`SELECT status, explanation, script_name, error_message FROM code_jobs WHERE id = $1 AND user_id = $2`, [jobId, userId]);
...
export async function getUserJobHistory(userId: string, limit = 20) {
  const { rows } = await db.query(`SELECT id, prompt, script_name, script_type, status, created_at, completed_at, inserted_at, insert_location FROM code_jobs WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2`, [userId, limit]);
  return rows;
}

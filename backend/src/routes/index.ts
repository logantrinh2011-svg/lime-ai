...
router.post('/auth/register', authRateLimit, async (req, res) => {
  ...
  try {
    const { userId, verifyToken } = await registerUser(req.body.email, req.body.password, req.body.username);
...
router.patch('/user/me', requireAuth, async (req, res) => {
  ...
  await db.query(
    `UPDATE users SET display_name = COALESCE($1, display_name),
       roblox_username = COALESCE($2, roblox_username), updated_at = NOW() WHERE id = $3`,
...
router.post('/chat', requireAuth, checkBanned, planUsageLimit, async (req, res) => {
  ...
  const { rows: planRows } = await db.query<{ max_tokens_per_request: number; requests_per_day: number; requests_per_month: number }>(
    `SELECT sp.max_tokens_per_request, sp.requests_per_day, sp.requests_per_month
     FROM users u JOIN subscription_plans sp ON u.plan_id = sp.id WHERE u.id = $1`, [userId]
  );
  ...
  try {
    ...
    await db.query(`INSERT INTO analytics_events (user_id, event_type, properties) VALUES ($1, 'chat_sent', $2)`,
...
router.post('/jobs', requireAuth, checkBanned, planUsageLimit, async (req, res) => {
  ...
  try {
    ...
    res.status(202).json({ jobId, message: 'Job created. Lychee AI is generating your code.' });
...
router.post('/plugin/heartbeat', requireAuth, async (req: any, res) => {
  try {
    await db.query(
...
router.get('/plugin/status', requireAuth, async (req: any, res) => {
  try {
